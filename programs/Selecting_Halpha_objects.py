...
cx, cy = colour(df, "r_psf", "i_psf", "r_psf", "J0660_psf")
...
data = pd.concat([df["RA_r"], df["DEC_r"], df["r_psf"], colum1, colum2], axis=1)
...
ecx, ecy = errormag(df, "e_r_psf", "e_i_psf", "e_r_psf", "e_J0660_psf")
...
with sns.axes_style('white'):
    ...
    ax1.plot(x_values, fitted_line_(x_values) + 4*sigma_fit, ls=':', color="k", zorder = 8, label='Representation 4$\sigma$')
    plt.fill_between(x_values, fitted_line_(x_values), fitted_line_(x_values) + 4*sigma_fit, color='k', alpha = 0.1)
    ax1.annotate(cmd_args.Ranger, xy=(3.8, 4.2),  xycoords='data', size=25,
    ...
    ax1.annotate("Median Errors", xy=(3.7, 3.75),  xycoords='data', size=25,
            xytext=(-120, -60), textcoords='offset points', )
